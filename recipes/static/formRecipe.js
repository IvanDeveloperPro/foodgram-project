...
const defineInitialIndex = function () {
    const ingredients = ingredientsContainer.querySelectorAll('.form__field-item-ingredients')
...
function Ingredients() {
    ...
    const addIngredient = (e) => {
        if(nameIngredient.value && cantidad.value) {
            ...
            elem.classList.add('form__field-item-ingredients');
            elem.id = `ing_${cur}`;
    ...
    const eventDelete = (e) => {
        if(e.target.classList.contains('form__field-item-delete')) {
            const item = e.target.closest('.form__field-item-ingredients');
